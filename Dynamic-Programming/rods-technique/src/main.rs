...
fn do_rods_technique(
    ...
    if current_value + remaining_value - items[next_index as usize].value > best_value {
        if items[next_index as usize + 1].block_list.iter().filter(|id| id == items[next_index].id).count() == 1 {
            items[next_index as usize + 1].blocked_by = items[next_index].id;
        }       
...
fn make_block_lists(items: &mut Vec<Item>) {
    for i in 0..items.len() {
        ...
        for j in 0..items.len() {
            if i != j && item[i].value >= item[j].value && item[i].weight <= item[j].weight {
                item[i].block_list.push(j);
            }
        }
    }
}